...
from utils import get_SVM_performance, get_pos_neg_masks
from losses import SupConLoss, ModifiedConLoss, ConvConLoss, SimCLRLoss
...
class BaseModel:
    ...
    def _load_best_model(self, fold):
        ...
        checkpoint = torch.load(
            Path(
                self.res_path, f'{self.method}--{self.loss}--{self.data.name}--{fold}--{self.val_metric}_checkpoint.pt'
            )
        )
    ...
    def _get_test_performance(self, train_X, train_y, test_X, test_y):
        ...
        self.fold_res['test_acc'] = res['test_acc']
...
class AFRCL(BaseModel):
    ...
    def _fit(self, fold, train_X, train_y, val_X, val_y):
        ...
        if self.loss == 'ModifiedSupCon':
            criterion = ModifiedConLoss(self.device, self.temperature)
        elif self.loss == 'SupCon':
            criterion = ConvConLoss(self.device, self.temperature)
        else:
            raise NotImplementedError(
                f'{self.loss} is not implemeneted. Please enter one of the following losses: '\
                'ModifiedSupCon or SupCon'
            )

        for epoch in range(self.epochs + 1):
            self.enc.train()

            for real_x, real_y in data_loader_:
                real_x = real_x.to(self.device)
                real_y = real_y.to(self.device)
                self.enc.zero_grad()

                pos_mask, neg_mask  = get_pos_neg_masks(real_y, self.device)
                _, proj_x = self.enc(real_x)
                loss = criterion(proj_x, pos_mask, neg_mask)
                loss.backward()
                self.opt.step()
    ...
    def _fit(self, fold, train_X, train_y, val_X, val_y):
        ...
        for epoch in range(self.epochs + 1):
            ...
            for x, y in data_loader_:
                ...
                x_i = x_i.requires_grad_(True).to(self.device)
                x_j = x_j.requires_grad_(True).to(self.device)
